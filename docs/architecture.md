...
`Routine` owns only ordered exercise structure (`RoutineExercise` objects); it is independent of people and contains no workout weights, reps, sets, or customizable units. Units resolve from the saved master `Exercise` record in routines, open workouts, history, and progress. `ExerciseLog` resolves its stable exercise UUID with a bounded lookup and transiently caches the model (not its unit); the stored `unitRaw` is retained only as a legacy/orphan fallback, so no persistent schema change is needed. Tapping the exercise name in a workout pushes the existing exercise detail screen, where editing the master unit immediately updates the workout on return. Duplicated exercises retain independent master records and units. `WorkoutSession` owns the selected participant set and its per-person exercise logs. When a new session starts, those logs are copied from each person's latest session for that routine, with legacy routine prescriptions used only as a migration fallback. The active-workout screen keeps participant selection in a safe-area-inset header so it remains available while the exercise list scrolls. An optional `deletedAt` timestamp implements lightweight soft deletion: active-routine queries exclude deleted rows, while Deleted Routines can clear the timestamp to restore the same routine graph.
...
Plank defaults to seconds and a 60-second target when no prior workout value exists. A one-off catalog correction changes the original bundled Plank record from reps to seconds on existing installs, without rewriting saved sets or repeatedly overriding later user edits. Timed exercises show one unit-labeled target control rather than load plus reps. Imported duration values retain canonical seconds, and time edits keep those values consistent with the displayed target. Existing numeric records, IDs, relationships, ordering, and completion state are preserved; changing between unrelated unit types corrects their interpretation rather than guessing a conversion.

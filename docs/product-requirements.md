...
- As a lifter, I want a celebratory completion screen showing each person's workout volume, new personal records, recent workout count, and recent volume, so that finishing a session feels rewarding and progress is immediately visible.
...
- As a lifter, I want pounds-only training-volume line charts separated by person and a compact time-range filter, so that I can compare weight-training trends without mixing incompatible units or crowding the page.
...
- The primary navigation is Workout, Routines, Exercises, Progress, and Settings; Settings contains People. Workout presents up to three routines as Quick Start with an All Routines path, the four-week activity second, and a conditional Deleted Workouts link last. Deleted workouts never appear in the activity calendar.
- The exercise library uses one compact filter menu ordered All, Custom, then an alphabetical Main Muscle section. Exercise creation describes its logging unit directly, keeps Main Muscle and Equipment visible, moves descriptive Exercise Type behind More Details, and explains how each field is used.
- Progress defaults to the last four weeks and exposes only a compact 4W/12W/All duration menu. Training Volume is a person-separated line chart with dots at each recorded session; history remains scoped to the selected duration.
- Completing a workout presents a celebration screen before returning to Workout home. It shows each active participant's completed-set count and pounds volume, highlights new personal records, and shows four-week workout and pounds-volume totals for each person with concise encouraging copy. The screen is dismissible back to Workout home.
